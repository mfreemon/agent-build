...
def get_stock_price(ticker:str) -> dict:
  ...
  try: 
    ...
    current_price = stock.info.get(current_price)

    if current_price is None:
      return {
        "status": "error",
        "error_message": f"Could not fetch stock data for {ticker}",
      }
    ...
    return {
      "status": "error",
      "error_message": f"Error fetching prices for {ticker}"
    }
...
stock_analyst = Agent(
    ...
    If the get_stock_price is unable to return a price, then use the news_analyst agent to perform the task
    """,
